...
public interface FavoriteRepository extends JpaRepository<Favorite, Long> {
  ...
  @Query("select "
      + "new kdt.prgrms.kazedon.everevent.domain.favorite.dto.SimpleMarketFavorite(m.id, m.name, m.favoriteCount) "
      + "from Favorite f join f.market m on f.user.id = :memberId")
  Page<SimpleMarketFavorite> findSimpleFavoriteByUserId(Long memberId, Pageable pageable);
}
